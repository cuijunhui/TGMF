...
# ---------------------- 提取特征函数 ----------------------
def extract_features(model, spectra, device, labels=None, batch_size=32):
    model.eval()
    features_list = []
    labels_list = []
    ...
    with torch.no_grad():
        for i in range(0, len(spectra), batch_size):
            ...
            features = model(spectra_tensor, return_features=True)
...
for fold, (train_idx, test_idx) in enumerate(kf.split(spectra, labels)):
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(X_train), batch_size):
            batch_spectra = X_train_tensor[i:i + batch_size]
        ...
        with torch.no_grad():
            for i in range(0, len(X_test), batch_size):
                batch_spectra = X_test_tensor[i:i + batch_size]
    ...
    # 提取训练后特征
    post_train_features, post_train_labels = extract_features(model, X_test, device, labels=y_test, batch_size=batch_size)
...
for i in range(num_classes):
    ...
    for f in range(n_splits):
        if len(fold_sensitivities[f]) > i:
            class_sensitivities.append(fold_sensitivities[f][i])
        else:
            class_sensitivities.append(0)
...
for i in range(num_classes):
    class_specificities = []
    for f in range(n_splits):
        if len(fold_specificities[f]) > i:
            class_specificities.append(fold_specificities[f][i])
        else:
            class_specificities.append(0)
...
plt.title("Average Confusion Matrix (GoogLeNet)", fontsize=16)
plt.xlabel("Predicted Labels", fontsize=14)
plt.ylabel("True Labels", fontsize=14)
plt.savefig("average_confusion_matrix_googlenet_5fold.png", bbox_inches='tight', dpi=1200)
plt.show()
...
# 计算类内和类间距离
intra_dist, inter_dist = compute_class_distances(best_post_train_features, best_post_train_labels)
print(f"Best fold (Fold {best_fold}) Post-train: Avg intra-class distance: {intra_dist:.4f}, Avg inter-class distance: {inter_dist:.4f}")
...
ax.set_title(f"UMAP After Training (GoogLeNet)", fontsize=16)
...
plt.savefig(f"GoogLeNet_umap_best_fold_{best_fold}.png", bbox_inches='tight', dpi=1200)
plt.show()
...
ax.set_title(f"UMAP After Training (GoogLeNet, Subsampled, Best Fold {best_fold})")
...
cbar.set_ticklabels(label_encoder.classes_)
cbar.ax.tick_params(rotation=45, labelsize=10)
plt.savefig(f"GoogLeNet_umap_subsampled_best_fold_{best_fold}.png", bbox_inches='tight')
plt.show()
...
scatter = ax.scatter(best_post_train_umap_combined[:, 0], best_post_train_umap_combined[:, 1],
                     c=best_post_train_labels, cmap='tab20', s=20, alpha=0.5)
ax.set_title(f"UMAP After Training (GoogLeNet + Synthetic Text Features, Best Fold {best_fold})")
ax.set_xlabel("UMAP Component 1")
ax.set_ylabel("UMAP Component 2")
cbar = fig.colorbar(scatter, ax=ax, orientation='horizontal', pad=0.15, fraction=0.05, aspect=50, shrink=0.8)
cbar.set_ticks(range(num_classes))
...
cbar.set_ticklabels(label_encoder.classes_)
cbar.ax.tick_params(rotation=45, labelsize=10)
plt.savefig(f"GoogLeNet_umap_with_synthetic_text_best_fold_{best_fold}.png", bbox_inches='tight', dpi=1200)
plt.show()
...
plt.tight_layout()
plt.savefig('loss_curves_googlenet_5fold.png')
plt.show()